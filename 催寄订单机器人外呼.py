...
def main():
    ...
    def date(para): # 该函数解决读取excel日期格式是数字的问题
        if type(para) != int:
            return para
        delta = pd.Timedelta(f'{int(para)}days')
        return (pd.to_datetime('1899-12-30')+delta).strftime('%m月%d日')
    ...
    # # 构建智齿外呼机器人所需的传入外呼客户格式,not_send_result
    # call_list = not_send_result[(~not_send_result.loc[:,'Fsender_phone'].isin(df_result['dn']))] #排除已经外呼接通过的电话号码
    ...
    # 构建智齿外呼机器人所需的传入外呼客户格式,order_booking_overtime
    ...
    writer1 = pd.ExcelWriter(file_path+ '\\催寄记录\\超过预约时间催寄\\' + '{}.xlsx'.format(today)) 
    # not_send_result.to_excel(writer1,'{}'.format(today),index=False,columns=not_send_result_columns)
    call_list.to_excel(writer1,'{}'.format(today),index=False)
