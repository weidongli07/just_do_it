...
def main():
    def send_msg(url, reminders,messge1 ):
        ...
        "at": {
            "atMobiles": ['17688938632','13246634506'], # 修改需要@的群内人员手机号
            "atUserIds":[],
            "isAtAll":False
        }
    }
    ...
    and o.Fvalid > 0 and (o.ftest = 0 or o.ftest is null) '''.format(the_7day_before,end_time)
    ...
    not_send_result_tb_str = str(table_srt(not_send_result[['来源','main_channel','Fsender_phone','寄件用户名',]]))
    new_payfail_tb_str = str(table_srt(new_payfail[new_payfail['外呼日期'] == today][['渠道','订单id','手机号','订单状态','付款失败提示']].reset_index(drop=True)))
    
    url1 = 'https://oapi.dingtalk.com/robot/send?access_token=2ecee5fc7688fbbdca942cc1c2f656446edea37ac63a420e354b499a02f4b41e'
    messge1 = "一、{0}催寄-催寄&付款失败\n".format(today) + not_send_result_tb_str
    messge2 = "二、{0}付款失败-催寄&付款失败\n".format(today) + new_payfail_tb_str
    reminders =[]
    print(send_msg(url1, reminders, messge1))
    print(send_msg(url1, reminders, messge2))
...
if __name__ == '__main__':
    ...
    # 定时周一到周日每天9.20播报
    sched.add_job(main, 'cron', hour='8', minute='50', second='00',day_of_week='0-6',coalesce=True, misfire_grace_time=3600)
    sched.start()
    etime = datetime.now()
