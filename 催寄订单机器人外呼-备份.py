...
Forder_id
,CONCAT('{{',
GROUP_CONCAT( concat('"Fremark_id":"' ,Fremark_id,'"',',',
'"detail":',
'{{',
'"Fstatus":"',Forder_status,'"',',',
'"Fremark":"',Fremark,'"',',',
'"Fupdate_time":"',Fupdate_time,'"',',',
'"Foperator_name":"',Foperator_name,'"'
,'}}'
)
),
'}}') as r_json
from recycle.t_order_remark
where  
Forder_status in (24,25,10) and Fstatus = 1
and Fremark like '%催寄%' and
substr(Fupdate_time,1,10) between '{0}' and '{1}'
group by Forder_id
...
#使用format时，如果原本文本就存在{或者}，可以把{用{{代替。
...
try:
    ...
    stime = datetime.now()
    print('开始查询sql')
    cursor = conn.cursor(cursor = pymysql.cursors.DictCursor) # 设置成DictCursor，结果包含字段名称
    cursor.execute('set group_concat_max_len=102400;')  #解决group_concat在合并字符串过长，导致自动截断的问题，设定最大长度
    cursor.execute(sql_order) #查询sql
    cds = cursor.fetchall() 
    order_all = pd.DataFrame(cds) #读取为DataFrame格式 
    cursor.close()
    conn.close()
    etime = datetime.now()
...
# read_sql、cursor游标区别：此处使用游标后读取为DataFrame格式
# read_sql ：只能执行查询数据
...
def json_get(data,l1,l2):# 该函数解析json
    if data is None:
        return np.nan
    else:
        try:
            json_result = json.loads(data,strict=False).get(l1).get(l2)
            return json_result
        except Exception as e:
            print(data,'Json解析错误:',e)
            # pass


detail['人工催寄备注'] = detail['人工催寄记录'].apply(lambda x:json_get(x,'detail','Fremark'))
detail['催寄时间'] = detail['人工催寄记录'].apply(lambda x:json_get(x,'detail','Fupdate_time'))
detail['人工催寄结果'] = detail['人工催寄备注'].apply(lambda x: np.nan if pd.isna(x)  else '未接' if re.match(r'(.*)(未接|停机|空号|关机|拒接)(.*)',x) else '已接通')
...
# 根据条件过滤得 过了预约上门时间超8小时未寄出回收订单
order_booking_tmp = detail[ (detail.loc[:,'下单状态'].isin(booking_status_list))
                       & (detail.loc[:,'回收类型'].isin(recycle_type_list))
                       & (detail.loc[:,'业务类型'].isin(business_mode))
                       & (detail.loc[:,'寄出时间'].isnull())
                       & (~detail.loc[:,'人工催寄结果'].isin(['已接通']))
                       & (~detail.loc[:,'预约上门时间'].isnull())
                       & ( detail.loc[:,'预约上门时间'] < (datetime.now() + timedelta( hours=-8)))     # 超8小时                       
                       & (~detail.loc[:,'Fsender_phone'].isin(black_phone_list)) ]
order_booking_overtime = order_booking_tmp.copy()
...
taskId = 'a6160b1cfca54e66a3c563514debfb04' #每日催寄任务

# taskId = '7534aeae589247ff8657ecdbafb118d5' #每日催寄任务1111
...
beginTime = round((time.time() - 7*86400)*1000) #7天前，转为毫米时间戳
endTime = round(time.time()*1000) #现在时间，转为毫米时间戳




pageSize = 50 #初始查询设定50条一页
data_dict_0 =  {
  "beginTime":beginTime, 
  "endTime":endTime, 
  "pageIndex":1, 
...
data_0 = json.dumps(data_dict_0)  #构建初始data,用于先获取总共多少页
task_json_0 = getresult.taskresult(companyId,taskId,data_0)
pageCount = task_json_0.get('content').get('pageCount') #获取该次查询结果共有多少页
totalCount = task_json_0.get('content').get('totalCount')
print('本次查询机器人外呼任务的结果总条数为：{0}条，共{1}页'.format(totalCount,pageCount))

result_data = []
for n in range(pageCount + 1)[1:]: #遍历页数
    data_dict =  {
                      "beginTime":beginTime, 
                      "endTime":endTime, 
                      "pageIndex":n, 
    ...
    task_json = getresult.taskresult(companyId,taskId,data)
    for i in task_json['content']['list']: #遍历外呼号码的返回结果
        dic = {}
        dic['outId']=i.get('detail').get('contactData').get('outId')
        dic['status']=i.get('detail').get('status')
        dic['startTime']=i.get('detail').get('startTime')
        dic['dn']=i.get('detail').get('dn')
        result_data.append(dic)

    

df_result_tmp = pd.DataFrame(result_data)
df_result = df_result_tmp.copy()
# df_result = df_result.query('status != 2') ## 筛选未接通的外呼 
df_result = df_result.query('status == 2') ## 筛选未接通的外呼，status == 2为已接通


# 构建智齿外呼机器人所需的传入外呼客户格式---昨日未呼出
# data_recall = df_result[['outId','dn']].rename({'dn':'telephone'},axis=1)
# phone_json_recall_tmp = data_recall.to_json(orient = 'records')
# phone_json_recall = json.dumps( {'contactList':json.loads(phone_json_recall_tmp)} )
# getresult.addnum(companyId,taskId,phone_json_recall)  #添加外呼数据


# # 构建智齿外呼机器人所需的传入外呼客户格式---下单长时间未寄出
# call_list = not_send_result[(~not_send_result.loc[:,'Fsender_phone'].isin(df_result['dn']))]
# data_phone = call_list[['forder_id','Fsender_phone']].rename({'forder_id':'outId','Fsender_phone':'telephone'},axis=1)
...
# 构建智齿外呼机器人所需的传入外呼客户格式---超预约上门时间未寄出8小时
call_list = order_booking_overtime[(~order_booking_overtime.loc[:,'Fsender_phone'].isin(df_result['dn']))] #排除已经外呼接通过的电话号码
...
#   ] 
# }
test = not_send_result.copy()
test['variable_json'] =  test[['main_channel','寄件用户名','预约上门时间','下单时间']].rename({'寄件用户名':'name','预约上门时间':'bookingtime','下单时间':'ordertime'},axis=1).to_json(orient = 'records',force_ascii=False)

# a = test[['forder_id','Fsender_phone']].rename({'forder_id':'outId','Fsender_phone':'telephone'},axis=1)
# b = a.to_json(orient = 'records',force_ascii=False)
test['variable'] =  test['variable_json'].apply(lambda x:json.dumps( {'variable':json.loads(x)} , ensure_ascii=False))
a1 =  test[['forder_id','Fsender_phone','variable']].rename({'forder_id':'outId','Fsender_phone':'telephone'},axis=1).to_json(orient = 'records',force_ascii=False)

d = json.dumps( {'contactList':json.loads(a1)} )
...
writer1 = pd.ExcelWriter(file_path+ '\\催寄记录\\' + '{}-1.xlsx'.format(today)) 
...
#     # 定时周一到周日每天9.20播报
#     sched.add_job(main, 'cron', hour='11', minute='55', second='00',day_of_week='0-6',coalesce=True, misfire_grace_time=3600)
#     sched.start()
